...
@section('content')
    <div class="d-flex flex-wrap align-items-center justify-content-between">
        <h5 class="fs-4 fw-bolder text-black">Edit Category</h5>
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.index') }}">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin.categories') }}">Categories</a></li>
                <li class="breadcrumb-item active text-black-50" aria-current="page">Edit Category</li>
            </ol>
        </nav>
    </div>

    <form method="POST" enctype="multipart/form-data" action="{{ route('admin.category.update') }}">
        @csrf
        @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-9 mt-3">
                    <input type="text" class="input-field w-100" placeholder="Category Slug" name="slug"
                        id="categorySlug" value={{ $category->slug }}>
                    @error('slug')
                        <span class="alert alert-danger text-center">{{ $message }}</span>
                    @enderror
                </div>
